...
- Azure Communication Services Email for invitations and completion notifications
...
- Public `/sign/*` routes
- Public `/api/public/*` signer API routes
- Authenticated `/api/*` admin routes
- Security headers
- Static SPA fallback to `index.html`

## 9. Configure Static Web Apps authentication

The app uses Microsoft Entra ID through Azure Static Web Apps authentication.

In the Static Web App resource:

1. Open **Authentication**.
2. Add Microsoft Entra ID as an identity provider.
3. Use the provider configuration generated by Static Web Apps where possible.
4. Add the production Static Web App URL as an allowed redirect URL if requested.
5. Add the local development URL only in a separate development app or test registration.

The front end enables its admin gate with:

```env
VITE_ENABLE_AUTH=true
```

The API independently validates the `x-ms-client-principal` header and requires an authenticated or admin role on admin routes. Do not rely only on the front-end gate.
...
6. Update `DOCULNK_PUBLIC_BASE_URL` on the Function App to the final HTTPS hostname.
...
6. Enter a real test recipient email and generate a request.
7. Confirm ACS delivers the invitation and the URL points to the production Static Web App host.
8. Open the signing link in a separate browser profile or private window.
...
12. Confirm the request becomes completed and the admin can download the Blob-backed signed PDF.
13. Confirm a second completion attempt is rejected.
14. Revoke a test request and confirm its signer URL is rejected.
15. Extend a request and confirm the new expiration is shown.
16. Verify audit events and Application Insights logs contain request IDs but no document contents.
...
- [ ] `DOCULNK_PUBLIC_BASE_URL` matches the real public host
