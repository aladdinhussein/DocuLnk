...
Each completed form is saved to Blob Storage before Azure Communication Services attempts delivery. The signed PDF is attached to an email sent to `DOCULNK_ADMIN_EMAIL`; if delivery fails, the submission remains available through the authenticated admin archive and the public download returned to the signer. The sender domain/address must be verified in the ACS resource.
